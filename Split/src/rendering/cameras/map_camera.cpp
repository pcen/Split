...
namespace Split
{
	...
	void MapCamera::set_sensitivity(float sensitivity)
	{
		m_sensitivity = sensitivity;
	}

	void MapCamera::set_speed(float speed)
	{
		m_speed = speed;
	}

	bool MapCamera::move_in_x(mouse_data& mouse)
	{
		return m_mouse_on_screen && ((int)mouse.x < m_move_threshold || m_screen_size.x - (int)mouse.x < m_move_threshold);
	}

	bool MapCamera::move_in_y(mouse_data& mouse)
	{
	...
	void MapCamera::update(double dt)
	{
		...
		mouse_data& mouse = Input::get_mouse();
	...
	void MapCamera::on_window_resize(WindowResize& resize)
	{
		m_screen_size = resize.dimensions();
	}

	void MapCamera::on_key_press(KeyPress& key)
	{
		std::cerr << "MapCamera: " << key.keycode() << " pressed\n";
	}

	void MapCamera::on_mouse_window_border(MouseWindowBorder& event)
	{
